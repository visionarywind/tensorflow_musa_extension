...
__global__ void ShiftedAffineMapKernel(
  ...
  for (int dim = shape.rank - 1; dim >= 0; --dim) {
    ...
    sliced_var_left_offset += coord * sliced_var_left_st.values[dim];
  ...
  const AccT left_val =
      static_cast<AccT>(LoadValue(sliced_var_left + sliced_var_left_offset));
  ...
  const AccT result = mask_val * (data_val + left_val) + right_val;
...
void LaunchShiftedAffineMapKernel(
  ...
  ShiftedAffineMapKernel<T><<<grid_size, block_size, 0, stream>>>(
      data_left, data_left_st, sliced_var_left, sliced_var_left_st, mask,
      mask_st, sliced_var_right, sliced_var_right_st, output, shape,
      total_elements);
}

template void LaunchShiftedAffineMapKernel<float>(
    const float*, ShiftedAffineMapStrides, const float*,
    ShiftedAffineMapStrides, const float*, ShiftedAffineMapStrides,
    const float*, ShiftedAffineMapStrides, float*, ShiftedAffineMapShape, int,
    musaStream_t);
template void LaunchShiftedAffineMapKernel<double>(
    const double*, ShiftedAffineMapStrides, const double*,
    ShiftedAffineMapStrides, const double*, ShiftedAffineMapStrides,
    const double*, ShiftedAffineMapStrides, double*, ShiftedAffineMapShape,
    int, musaStream_t);
template void LaunchShiftedAffineMapKernel<Eigen::half>(
    const Eigen::half*, ShiftedAffineMapStrides, const Eigen::half*,
    ShiftedAffineMapStrides, const Eigen::half*, ShiftedAffineMapStrides,
...
template void LaunchShiftedAffineMapKernel<bfloat16>(
    const bfloat16*, ShiftedAffineMapStrides, const bfloat16*,
    ShiftedAffineMapStrides, const bfloat16*, ShiftedAffineMapStrides,
