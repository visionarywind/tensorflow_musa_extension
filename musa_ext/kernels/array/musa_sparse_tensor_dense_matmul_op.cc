...
#include "tensorflow/stream_executor/musa/musa_stream.h"
...
class MusaSparseTensorDenseMatMulOp : public OpKernel {
 public:
  explicit MusaSparseTensorDenseMatMulOp(OpKernelConstruction* context) : OpKernel(context) {
    OP_REQUIRES_OK(context, context->GetAttribute("adjoint_a", &adjoint_a_));
    OP_REQUIRES_OK(context, context->GetAttribute("adjoint_b", &adjoint_b_));
  }
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES(context, values.dim_size(0) == nnz,
                errors::InvalidArgument("Values size must match indices size: ", values.dim_size(0), " vs ", nnz));
    OP_REQUIRES(context, dense_rows == K,
                errors::InvalidArgument("Dense matrix rows must match sparse matrix cols: ", dense_rows, " vs ", K));
    ...
    // 注意：allocate_temp 分配的内存生命周期仅在当前 Compute 调用期间有效，无需手动 free
    ...
    OP_REQUIRES_OK(context, context->allocate_temp(
        DT_INT32, TensorShape({M}), &d_row_counts_tensor));
    
    // 分配 row_ptr: [M+1] int64
    OP_REQUIRES_OK(context, context->allocate_temp(
        DT_INT64, TensorShape({M + 1}), &d_row_ptr_tensor));
        
    // 分配 col_idx: [nnz] int64
    OP_REQUIRES_OK(context, context->allocate_temp(
        DT_INT64, TensorShape({nnz}), &d_col_idx_tensor));
        
    // 分配 csr_values: [nnz] float
    OP_REQUIRES_OK(context, context->allocate_temp(
        DT_FLOAT, TensorShape({nnz}), &d_csr_values_tensor));

    // 分配 row_ptr_copy: [M+1] int64 (用于 FillCSR 内部修改)
    OP_REQUIRES_OK(context, context->allocate_temp(
        DT_INT64, TensorShape({M + 1}), &d_row_ptr_copy_tensor));
    ...
    // 注意：同步操作会影响性能，但如果必须要在 Host 做扫描，这是不可避免的开销。
    // 更优解是实现 Device 端的 Prefix Sum Kernel，但此处仅针对内存分配优化。
    ...
    musaMemcpy(h_row_counts.data(), d_row_counts, M * sizeof(int32_t), musaMemcpyDeviceToHost);
    ...
    musaMemcpyAsync(d_row_ptr, h_row_ptr.data(), (M + 1) * sizeof(int64_t), musaMemcpyHostToDevice, raw_stream);
    
    // 拷贝到 copy buffer
    musaMemcpyAsync(d_row_ptr_copy, d_row_ptr, (M + 1) * sizeof(int64_t), musaMemcpyDeviceToDevice, raw_stream);

    // Step 6.3: 填充 CSR 的 col_idx 和 values
    LaunchFillCSR(
        indices.flat<int64_t>().data(),
        values.flat<float>().data(),
        nnz,
        d_row_ptr_copy, // 使用 copy，因为 FillCSR 可能会修改它（如果是原子加逻辑）
        d_col_idx,
        d_csr_values,
        raw_stream);
    ...
    // 8. 调用核心矩阵乘法核函数
    LaunchSparseDenseMatMul(
        d_row_ptr,
        d_col_idx,
        d_csr_values,
        dense.flat<float>().data(),
        M, K, N,
    ...
    // Tensor 对象在函数退出时自动销毁，底层内存由 TensorFlow  allocator 管理回收。
    
    // 10. 检查错误
    musaError_t err = musaGetLastError();
    OP_REQUIRES(context, err == musaSuccess,
                errors::Internal("MUSA SparseTensorDenseMatMul kernel launch failed: ", musaGetErrorString(err)));
  }
